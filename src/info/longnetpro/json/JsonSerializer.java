...
public class JsonSerializer {
    ...
    public JsonSerializer(Writer out, JsonFormat format) {
        super();
        this.out = out;
        this.format = format;
    }
    ...
    public static void serialize(JsonType jsonObj, Writer out, JsonFormat format) throws IOException {
    ...
    public static String toJsonString(String string, boolean hexUpperCase, boolean hexSpecialChar, boolean hexNonAscii, boolean hexAscii,
                                      boolean escapeSlash) {
        StringBuilder sb = new StringBuilder("");
        sb.append('"');
        for (int i = 0; i < string.length(); i++) {
            ...
            if (JsonParser.isSpecialCharacter(ch)) {
                // special
                s = hexSpecialChar ? "\\u" + StringUtils.toHexString(ch, hexUpperCase) : escapeSpecialChar(ch, escapeSlash);
    ...
    private String toJsonString(String string) {
        ...
        if (format.isHexAllCharacters()) {
            ...
            s = toJsonString(string, format.isHexUpperCase(), format.isHexSpecialChar(), format.isHexNonAscii(), format.isHexAscii(), format.isEscapeSlash());
        }
        return s;
    }
